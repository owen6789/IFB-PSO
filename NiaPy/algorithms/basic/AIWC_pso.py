...
__all__ = ['AIWCParticleSwarmAlgorithm']


class AIWCParticleSwarmAlgorithm:
    ...
    **Reference paper:**
    ...
    def move_particles(self):
        ...
        while self.eval_flag is not False:
            ...
            self.P_t = self.P_t / self.NP
            # update w:
            self.w = (self.w_max - self.w_min) * self.P_t + self.w_min
            # update a1 and a2:
            self.a1 = self.alpha * self.P_t + self.beta
            self.a2 = self.alpha * (1 - self.P_t) + self.beta
            ...
            for i in range(self.NP):
                ...
                for j in range(self.D):
                    ...
                    self.Velocity[i][j] = (self.w * self.Velocity[i][j])+ \
                        (self.a1 * random.random() * (self.pBestSolution[i][j] - self.Solution[i][j])) + \
                        (self.a2 * random.random() * (self.gBestSolution[j] - self.Solution[i][j]))
