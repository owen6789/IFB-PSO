...
#attracted by three points 
__all__ = ['TPABackwardParticleSwarmAlgorithm']


class TPABackwardParticleSwarmAlgorithm:
    ...
    **Reference paper:**
    ...
    def __init__(self, D, NP, nFES, C1, C2, w, vMin, vMax, benchmark,imp_check,strategy):
        ...
        #inverse check: if==0 normal particle, if=1 inverse particle
        self.inv_check = numpy.zeros([self.NP])
        self.backward_position = numpy.zeros([self.NP,self.D])
        self.C3 = 1.5
            ...
            for i in range(self.NP):
                ...
                if self.iters_check[i] >= self.imp_check:
                    ...
                    if self.strategy == 1:
                        ...
                        self.backward_position[i] = X[p_no]
                ...
                if self.inv_check[i] == 1:
                    for j in range(self.D):
                        self.Velocity[i][j] = (self.w * self.Velocity[i][j])+ \
                            (1.5 * random.random() * (self.gBestSolution[j] - self.Solution[i][j])) + \
                            (1.5 * random.random() * (self.backward_position[i][j] - self.Solution[i][j]))
                        
                        # self.Velocity[i][j] = (self.w * self.Velocity[i][j])+ \
                        #     (1 * random.random() * (self.pBestSolution[i][j] - self.Solution[i][j])) + \
                        #     (1 * random.random() * (self.gBestSolution[j] - self.Solution[i][j])) + \
                        #     (1 * random.random() * (self.backward_position[i][j] - self.Solution[i][j]))
