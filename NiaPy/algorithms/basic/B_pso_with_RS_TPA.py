# encoding=utf8
# backward PSO with random search(RS)
import random
import numpy
from NiaPy.benchmarks.utility import Utility
from sklearn.ensemble import IsolationForest

__all__ = ['TPARSBackwardParticleSwarmAlgorithm']


class TPARSBackwardParticleSwarmAlgorithm:
            ...
            for i in range(self.NP):
                ...
                if self.iters_check[i] >= self.imp_check:
                    ...
                    if self.strategy == 1:
                        ...
                        #Random Search Part:
                        backward_p = X[p_no]
                        backward_fit = self.Fun(self.D, X[p_no])
                        ...
                        for k in range(len(candidate_p)):
                            ...
                            for j in range(self.D):
                                ...
                                candidate_p[k][j] = X[p_no][j] + (self.Upper - self.Lower)*random.uniform(-0.2,0.2) 
                        ...
                        #check bound
                        self.Solution[i] = self.bounds(self.Solution[i])
                        self.backward_position[i] = self.Solution[i]
                ...
                if self.inv_check[i] == 1:
                    for j in range(self.D):
                        ...
                        self.Velocity[i][j] = (self.w * self.Velocity[i][j])+ \
                            ...
                            (self.C3 * random.random() * (self.backward_position[i][j] - self.Solution[i][j]))
