...
__all__ = ['SNoPVBackwardParticleSwarmAlgorithm']


class SNoPVBackwardParticleSwarmAlgorithm:
    ...
    **Reference paper:**
    ...
    def __init__(self, D, NP, nFES, C1, C2, w, vMin, vMax, benchmark,imp_check,strategy):
        ...
        #state: 1. G_best 2. P_best 3. current fitness  4. Velocity
        self.state_his = numpy.zeros([self.NP,self.iters, self.D + 3])
    ...
    def move_particles(self):
        ...
        while self.eval_flag is not False:
            ...
            for i in range(self.NP): #i: no. of particle
                ...
                #record velocity
                self.state_his[i][iters_count][0:self.D] = self.Velocity[i]
                #Gbest
                self.state_his[i][iters_count][(self.D)] = self.gBestFitness
                #Pbest
                self.state_his[i][iters_count][(self.D+1)] = self.pBestFitness[i]
                #current fitness
                self.state_his[i][iters_count][(self.D+2)] = Fit
            ...
            for i in range(self.NP):
                ...
                if self.iters_check[i] >= self.imp_check:
                    ...
                    if self.strategy == 1:
                        ...
                        self.Solution[i] = self.p_his[i][p_no+1]
