...
__all__ = ['IACParticleSwarmAlgorithm']


class IACParticleSwarmAlgorithm:
    ...
    **Reference paper:**
    ...
    def __init__(self, D, NP, nFES, C1, C2, w, vMin, vMax, benchmark):
        ...
        # parameters of PSO-IAC
        self.iter_max = self.iters
        self.w_max = 0.9
        self.w_min = 0.4
        self.P_t = 0
        self.P_t_past = 0
        self.chi = 2 / abs(2-4.1-math.sqrt(4.1*(4.1-4))) # X
        self.Qia = 0
    ...
    def move_particles(self):
        ...
        while self.eval_flag is not False:
            ...
            # update Qia, X(chi) and w:
            if iters_count == 0:
                self.w = 1
            else:
                self.Qia = self.w * (self.P_t - self.P_t_past)
                self.w = (self.w_max - self.w_min) * self.P_t - self.Qia
            ...
            for i in range(self.NP):
                ...
                for j in range(self.D):
                    ...
                    self.Velocity[i][j] = self.chi * ((self.w * self.Velocity[i][j])+ \
