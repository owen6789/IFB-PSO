...
class GeneticAlgorithm:
    ...
    **License:** MIT
    ...
    def __init__(self, D, NP, nFES, Ts, Mr, gamma, benchmark):
        ...
        self.Best = Chromosome(self.D, self.Lower_D, self.Upper_D)
    ...
    def CrossOver(self, parent1, parent2):
        ...
        alpha = [-self.gamma + (1 + 2 * self.gamma) * rnd.random()
                 for i in range(self.D)]
        child1 = Chromosome(self.D, self.Lower_D, self.Upper_D)
        child2 = Chromosome(self.D, self.Lower_D, self.Upper_D)
    ...
    def Mutate(self, child):
        ...
        for i in range(self.D):
            if rnd.random() < self.Mr:
                sigma = 0.20 * float(child.UB[i] - child.LB[i])
                child.Solution[i] = min(
                    max(rnd.gauss(child.Solution[i], sigma), child.LB[i]), child.UB[i])

    def init(self):
        """Initialize population."""
        for i in range(self.NP):
            self.Population.append(Chromosome(self.D, self.Lower_D, self.Upper_D))
